			...
			<div style="overflow: hidden !important;display: flex; flex-wrap: wrap;align-content: center;width: {{$barcode_details->width * 1}}in; height: {{$barcode_details->height * 1}}in;">
				...
				<div>
					...
					@if(!empty($print['business_name']))
						<b style="display: block !important; font-size: {{23*$factor}}px">{{$business_name}}</b>
					@endif
					...
					@endif
					 <small style="display: block !important; font-size: {{20*$factor}}px">{{$page_product->sub_sku}}</small>
					{{-- Barcode --}}
					<img style="max-width:90% !important;height: {{$barcode_details->height*0.24}}in !important;" src="data:image/png;base64,{{DNS1D::getBarcodePNG($page_product->sub_sku, $page_product->barcode_type, 2,30,array(39, 48, 54), false)}}">

					{{-- Price --}}
					@if(!empty($print['price']))
					<span style="display: block !important; font-size: {{22*$factor}}px">
